...
You can use the proportion of the points that are contained within the unit circle centered at the origin
to estimate the ratio of the area of the circle to the area of the square.
Given that you know the true ratio to be π/4, you can multiply the estimated ratio by 4 to approximate the value of π.
The more points that you sample to calculate this approximation, the closer the value should be to the true value of π.

.. image:: ../images/monte_carlo_pi.png

This tutorial uses Ray :ref:`tasks <ray-remote-functions>` to distribute the work of sampling and Ray :ref:`actors <ray-remote-classes>` to track the progress of these distributed sampling tasks.
...
To get started, install Ray using ``pip install -U ray``. See :ref:`Installing Ray <installation>` for more installation options.
...
To define a Ray actor, decorate a normal Python class with :func:`ray.remote <ray.remote>`.
The progress actor has a ``report_progress()`` method that sampling tasks call to update their progress individually
...
-------------------------
Once the actor is defined, you can create an instance of it.
...
------------------------
Now the task is defined, you can execute it asynchronously.
...
--------------------------
While sampling tasks are running, you can periodically query the progress by calling the actor ``get_progress()`` method.
...
As you can see from the preceding code, besides a single ``ObjectRef``, :func:`ray.get() <ray.get>` can also take a list of ``ObjectRef`` and return a list of results.
