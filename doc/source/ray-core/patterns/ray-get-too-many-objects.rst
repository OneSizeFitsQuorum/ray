...
**Summary:** Avoid calling :func:`ray.get() <ray.get>` on too many objects since this leads to heap out-of-memory or object store out-of-space. Instead fetch and process one batch at a time.
...
Instead you should get and process the results one batch at a time. Once Ray processes a batch, it evicts objects in that batch to make space for future batches.
...
Here besides getting one batch at a time to avoid failure, you're also using ``ray.wait()`` to process results in the finish order instead of the submission order to reduce the runtime. See :doc:`ray-get-submission-order` for more details.
